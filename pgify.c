...
pPgifyTree pgify(pANTLR3_INPUT_STREAM input, WalkerState state) {
    ...
    lxr->pLexer->rec->state->userp = &(state->options);
    ...
    psr->pParser->rec->state->userp = &(state->options);
...
gchar* pgify_string(pANTLR3_INPUT_STREAM input, WalkerState ws) {
    int tokenIndex = 0;
    GString *output = g_string_new("");
    pPgifyTree pt = pgify(input, ws);
    if(!pt)
        return NULL;
...
WalkerState walkerstate_new(gint pgoptions) {
    WalkerState ws = (WalkerState) malloc(sizeof(struct walker_state_struct));
    if(ws) {
        ws->found_rows = 1;
        ws->schemaName = g_strdup("");
        ws->postbuf    = g_string_new("");
        ws->psr        = 0;
        ws->options    = pgoptions;
    }
...
void walkerstate_free(WalkerState ws) {
    g_free(ws->schemaName);
    g_string_free(ws->postbuf, TRUE); 
    free(ws);
}
...
static void show_tables_rewrite(WalkerState ws, pANTLR3_BASE_TREE base) {
    ...
    gstatement = g_strdup_printf(show_tables, gcolumns, gschema, gschema, gwhere);
...
static void rewrite_found_rows(WalkerState ws, pANTLR3_BASE_TREE tree) {
    pANTLR3_COMMON_TOKEN token;
    pANTLR3_STRING str;
    gchar *expr;
    ...
    expr = g_strdup_printf("%"G_GINT64_FORMAT" AS \"found_rows()\"", ws->found_rows);
...
static void TreeWalkWorker(WalkerState ws, pANTLR3_BASE_TREE p, int level) {
    ...
    for(c = 0; c < p->getChildCount(p); c++) {
        ...
        else if(tokenType == K_FOUND_ROWS)
            rewrite_found_rows(ws, child);
    }
}
